...
///////////////////////Improvement///////////////////////////////////
//removed from the ready queue until they are done sleeping at which point they should be returned 
//to the ready queue
void sleep(int seconds) {
  int x = seconds * 125;
  setKernelDataSegment();
  running->state = SLEEPING;
  running->sleepCounter = x;
  restoreDataSegment();
...
void handleTimerInterrupt(int segment, int stackPointer) {
  ...
  //update the sleepCounter
  for(i=0; i<8; i++){
    if(pcbPool[i].state == SLEEPING){
      pcbPool[i].sleepCounter--;
      if(pcbPool[i].sleepCounter <=0){
  ...
  if(running->state != DEFUNCT && !stringMatch(running->name, "IDLE\0",5) && running->state != SLEEPING){
    running->state = READY;
    addToReady(running);
  }
...
int kill(int segment) {
  ...
  seg = (segment+2) *0x1000;
  setKernelDataSegment();
  for (i = 0; i < 8; i++) {
    if(pcbPool[i].segment == seg) {
